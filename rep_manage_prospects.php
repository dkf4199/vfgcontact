...
<title>VFG Contact - Manage Prospects</title>
<!-- CSS -->
			...
			<?php
				...
				if ($r) { // If it ran OK, display the records.
					...
					echo '<table id="csstable">
						<tr>
							<th colspan=8>YOUR PROSPECTS ('.$_SESSION['total_recs'].' total)<br /><br />* indirect contact</th>
						</tr>
						<tr>
							...
							<th scope="col"><a href="rep_manage_prospects.php?role=rep&sort=entrydt&sortorder='.$sort_order.'">Entry Date</a></th>
					...
					while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
						...
						echo '<tr class="'.$rowspec.'">
								<td><div id="contact-form"><a href="rep_edit_contact.php?s='.$start.'&cid='.$row['contact_id'].'&role=rep" class="bluelink" >Edit</a></div>
					...
					if ($pages > 1) {
						...
						if ($current_page != 1) {
							echo '<a href="rep_manage_prospects.php?s='.($start - $display).'&p='.$pages.'">Previous</a>&nbsp;';
						...
						for ($i = 1; $i <= $pages; $i++){
							if ($i != $current_page){
								echo '<a href="rep_manage_prospects.php?s='.(($display * ($i - 1))).'&p='.$pages.'">'.$i.'</a>&nbsp;';
						...
						if ($current_page != $pages){
							echo '&nbsp;<a href="rep_manage_prospects.php?s='.($start + $display).'&p='.$pages.'">Next</a>';
							//echo '<a href="rep_view_contact.php?&s='.($start + $display).'&p='.
							//	$pages.'">Next</a> ';
						}
					...
					//refresh button
					echo '<p align="center">
						<form action="rep_manage_prospects.php" method="GET" >
