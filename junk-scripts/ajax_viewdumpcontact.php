...
session_start();
	...
	$q = "SELECT a.firstname, a.lastname, a.email, a.phone,
		  ...
		  FROM dumped_contacts a INNER JOIN tierstatus_lookup b ON a.tier_status = b.tier_status
	...
	if ($r) { // If it ran OK, display the record.
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
			...
			echo '<p align="center">
					<span class="editheader">Restore Contact: '.$row['firstname'].' '.$row['lastname'].'</span></p>';
			
			$displayform = 
					'<div class="dumpdiv">
						
						<form name="viewdumpcontact_form" id="viewdumpcontact_form" onSubmit="return restoreContact()" >
							<div id="timedivwrap">
							...
							<ul>
								...
								<li>
									...
									<input type="submit" class="button" value="Restore" />
								</li>
								<li>
									<div id="ajax_verify_update"></div>
								</li>
							</ul>
							<p align="center">Restoring a contact will move it out of your "Dumped Contacts" list.<br />
									You can then find the contact on the "Manage Contacts" page.</p>
						</form>
					</div>';
			echo $displayform;
		}
