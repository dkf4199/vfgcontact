...
session_start();
	...
	// Make the query:
	$q = "SELECT contact_id, firstname, lastname, email, phone, 
				tier_status, entry_date, contact_type FROM contacts
		  WHERE rep_id = '$repsid'
		  AND team_member = 'N'
		  ORDER BY $order_by 
		  LIMIT $start, $display";
	...
	if ($r) { // If it ran OK, display the records.
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
			...
			echo '<tr class="'.$rowspec.'">
					<td><div id="contact-form"><a href="'.$row['contact_id'].'" class="contact">Edit</a>
						&nbsp;<a href="javascript: contactDumpInfo(\''.$row['contact_id'].'\');">Dump</a></div></td>
					<td>'.$row['lastname'].'</td>';
