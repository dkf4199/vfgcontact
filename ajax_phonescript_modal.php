...
session_start();
	...
	$contact_tz = $_GET['contacttimezone'];
	$tzdesc = '';
	...
	$displayform = '<div style="display:none">
		...
		<h1 class="contact-title">Phone Call Scripts for '.$fname.' '.$lname.' ('.$contactphone.' '.$tzdesc.' time)</h1>
			...
			<ul>
				<li>
					...
					<select name="script_list" id="script_list" onChange="javascript: getPhoneScript(this.value);">
						...
						<option value="1">Intro Call #1</option>
						<option value="2">Intro Call #2</option>
						<option value="3">Final Intro Call</option>
					</select>
