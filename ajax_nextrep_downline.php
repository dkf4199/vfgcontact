...
session_start();
	...
	$rep_firstname = $rep_lastname = $rep_vfgid = '';
	
	// Get this rep's data from the reps table
	// (need his/her firstname, lastname, vfgrepid)
	$q = "SELECT firstname, lastname, vfgrepid
		  FROM reps
		  WHERE rep_id='$repid' LIMIT 1";
	$r = @mysqli_query($dbc, $q);
	if (mysqli_num_rows($r) == 1){
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
			$rep_firstname = $row['firstname'];
			$rep_lastname = $row['lastname'];
			$rep_vfgid = $row['vfgrepid'];
		}
	...
	if (isset($_GET['p']) && is_numeric($_GET['p'])) {
		...
		//
		 $q = "SELECT count(rep_id) FROM reps
			   WHERE recruiter_vfgid='$rep_vfgid'";
		 $r = @mysqli_query($dbc, $q);
	...
	if ($r) { // If it ran OK, display the records.
		...
		if ($pages > 1) {
			...
			if ($current_page != 1) {
				echo '<a href="javascript: pageResults(\''.$rep_vfgid.'\',\''.$rep_firstname.'\',\''.$rep_lastname.'\','.($start - $display).','.$pages.');">Previous</a>&nbsp;';
				//echo '<a href="rep_manage_team_reps.php?s='.($start - $display).'&p='.$pages.'">Previous</a>&nbsp;';
			}
			//make number links to pages
			for ($i = 1; $i <= $pages; $i++){
				if ($i != $current_page){
					echo '<a href="javascript: pageResults(\''.$rep_vfgid.'\',\''.$rep_firstname.'\',\''.$rep_lastname.'\','.(($display * ($i - 1))).','.$pages.');">'.$i.'</a>&nbsp;';
			...
			if ($current_page != $pages){
				echo '&nbsp;<a href="javascript: pageResults(\''.$rep_vfgid.'\',\''.$rep_firstname.'\',\''.$rep_lastname.'\','.($start + $display).','.$pages.');">Next</a>';
