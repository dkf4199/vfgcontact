...
$_SESSION['from_page'] = 'rep_manage_customers.php';
$_SESSION['back_btn_text'] = 'Back To Manage Customers';
...
<title>VFG Contact - Manage Customers</title>
<!-- CSS -->
			...
			<?php
				...
				if ($r) { // If it ran OK, display the records.
					...
					echo '<table id="csstable">
						<tr>
							<th colspan=8>YOUR CUSTOMERS ('.$_SESSION['total_recs'].' total)<br /><br />* indirect contact</th>
						</tr>
						<tr>
							...
							<th scope="col"><a href="rep_manage_customers.php?sort=entrydt&sortorder='.$sort_order.'">Entry Date</a></th>
					...
					if ($pages > 1) {
						...
						if ($current_page != 1) {
							echo '<a href="rep_manage_customers.php?s='.($start - $display).'&p='.$pages.'">Previous</a>&nbsp;';
						...
						for ($i = 1; $i <= $pages; $i++){
							if ($i != $current_page){
								echo '<a href="rep_manage_customers.php?s='.(($display * ($i - 1))).'&p='.$pages.'">'.$i.'</a>&nbsp;';
						...
						if ($current_page != $pages){
							echo '&nbsp;<a href="rep_manage_customers.php?s='.($start + $display).'&p='.$pages.'">Next</a>';
							//echo '<a href="rep_view_contact.php?&s='.($start + $display).'&p='.
							//	$pages.'">Next</a> ';
						}
					...
					//refresh button
					echo '<p align="center">
						<form action="rep_manage_customers.php" method="GET" >
