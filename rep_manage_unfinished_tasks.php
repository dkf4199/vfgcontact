...
$_SESSION['from_page'] = 'rep_manage_unfinished_tasks.php';
$_SESSION['back_btn_text'] = 'Back To Unfinished Tasks';
...
<title>VFG Contact - Unfinished Tasks</title>
<!-- CSS -->
...
<div id="wrapper">
	...
	<div class="maincontent">
		...
		<?php
			...
			$thisday = date("Y-m-d");	//next_action_date is YYYY-MM-DD as well....for comparison
			...
			$r = @mysqli_query($dbc, $q);
			$row = @mysqli_fetch_array($r, MYSQLI_NUM);
			$_SESSION['total_unfinished_recs'] = $row[0];
			mysqli_free_result($r);
			
			// Make the query - using a UNION to get manager and consultant roles:
			$q = "SELECT 1 as 'rank', contact_id, firstname, lastname, email, next_action_date as 'action_date',
						 phone, 'Inviter' as role, 'none' as 'assigned_vfgid', rep_action_taken as 'action_taken', 
						 'none' as 'assigner_vfgid', 'Me' as 'fullname'
				  FROM contacts 
				  WHERE rep_id = '$repsid'
				  AND rep_action_taken = 'N'
				  AND next_action_date < '$thisday' 
				  UNION
				  SELECT 2 as 'rank', a.contact_id, a.firstname, a.lastname, a.email, a.next_action_date as 'action_date',
				  ...
				  AND a.manager_action_taken = 'N'
				  AND a.next_action_date < '$thisday' 
				  UNION
				  SELECT 3 as 'rank', a.contact_id, a.firstname, a.lastname, a.email, a.next_action_date as 'action_date',
				  ...
				  AND a.consultant_action_taken = 'N'
				  AND a.next_action_date < '$thisday' 
				  UNION
				  SELECT 4 as 'rank', a.contact_id, a.firstname, a.lastname, a.email, a.next_action_date as 'action_date',
				  ...
				  AND a.addlrep_action_taken  = 'N'
				  AND a.next_action_date < '$thisday' 
				  ORDER BY rank, action_date
				  LIMIT $start, $display";	
			...
			if ($r) { // If it ran OK, display the records.
				...
				echo '<table id="csstable">
					<tr>
						<th colspan=8>UNFINISHED TASKS</th>
					</tr>
					<tr>
				...
				while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
					...
					switch($row['role']){
						case 'Inviter':
							echo '<td><div id="contact-form"><a href="rep_edit_contact.php?s='.$start.'&cid='.$row['contact_id'].'&role=rep" class="bluelink" >Edit</a></div>';
							break;
						case 'Manager':
				...
				if ($pages > 1) {
					...
					if ($current_page != 1) {
						echo '<a href="rep_manage_unfinished_tasks.php?s='.($start - $display).'&p='.$pages.'">Previous</a>&nbsp;';
					...
					for ($i = 1; $i <= $pages; $i++){
						if ($i != $current_page){
							echo '<a href="rep_manage_unfinished_tasks.php?s='.(($display * ($i - 1))).'&p='.$pages.'">'.$i.'</a>&nbsp;';
					...
					if ($current_page != $pages){
						echo '&nbsp;<a href="rep_manage_unfinished_tasks.php?s='.($start + $display).'&p='.$pages.'">Next</a>';
						//echo '<a href="rep_view_contact.php?&s='.($start + $display).'&p='.
						//	$pages.'">Next</a> ';
					}
				...
				//refresh button
				echo '<p align="center">
					<form action="rep_manage_unfinished_tasks.php" method="GET" >
				...
				// Public message:
				echo '<p>Data could not be retrieved. We will fix the problem shortly.</p>';
				//echo '</div>';
