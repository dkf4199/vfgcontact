...
session_start();
	...
	if ($sendmail == 'Y') {
		...
		// Make the query to get matching email template for phone script:
			...
			if (mysqli_num_rows($r) == 1){
				...
				*/
				...
				$embody = '<p>'.$sal.'<br /><br />'.$body.'</p>
						<p>'.$close.'<br /><br />'.$repfn.' '.$repln.'<br />'.$repgmail.'<br />
				...
				try {
					...
					echo "Failed to send the Template! " . $ex->getMessage() . "<br />";
					$mail_sent = false;
				}
		...
		if ($r) { // If it ran OK, get the data if there is a record.
			if (mysqli_num_rows($r) == 1){
				...
				if ($dialer_unique_id != ''){
					...
					//Write text sent to the comm history for rep
					date_default_timezone_set($_SESSION['rep_tz']);
					$rightnow = date("Y-m-d H:i:s");
