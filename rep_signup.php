...
<div id="wrapper">
	...
	<div class="maincontent">
	  ...
	  <?php
		...
		$messages = array();		//initialize message array
		$messages[] = 'Fill out form to register.';
		...
		if (isset($_POST['submitted']) && $_POST['submitted'] == "register"){
			...
			$duplicate_gmail = false;	// dup gmail acct flag
			...
			if (empty($_POST['vfgrepid'])){
				...
				if (mysqli_num_rows($rs) == 1) {
					...
					$errors[] = 'VFG ID already exists in database.';
				}
				mysqli_free_result($rs);
			}
			
			// VFGREPID TYPE
			$vfgidtype = $_POST['vfgtype'];
			...
			if (empty($_POST['gmail_acct'])){
				...
				if (mysqli_num_rows($rs) == 1) {
					//email already exists!
					$errors[] = 'Gmail address already exists in another rep\'s record.';
					$duplicate_gmail = true;
					$_SESSION['duplicate_gmail'] = $gmail_acct;
				}
				mysqli_free_result($rs);
			...
			if (empty($errors)){
				...
				//Create Unique ID for this rep - 5 digit number
				$idexists = true;
				do {
				...
				$q = "INSERT INTO reps (rep_id, firstname, lastname, phone, rep_timezone, 
										signup_date, vfgrepid, vfgrepid_type, recruiter_vfgid, gmail_acct, gmail_pass,
				...
				mysqli_stmt_bind_param($stmt, 'ssssssssssssssss', $repid, $fn, $ln, $phone, $tz, $rightnow, 
															$vfgrepid, $vfgidtype, $recruiter_vfgrepid, $gmail_acct, $gmail_pass, 
				...
				if (mysqli_stmt_affected_rows($stmt) == 1) {	//rep data insert successful
					...
					*/
					...
					// Create the encrypted password hash
					$password_hash = better_crypt($passwrd);	// better_crypt - blowfish
					
					//prepared statement - INSERT DATA into rep_logins
					$lq = "INSERT INTO rep_login_id (rep_id, vfgid, password, pwd) 
						VALUES (?, ?, ?, ?)";

					//prepare statement
					$lstmt = mysqli_prepare($dbc, $lq);
					//bind variables to statement
					mysqli_stmt_bind_param($lstmt, 'ssss', $repid, $vfgrepid, $passwrd, $password_hash);
					//execute query
					...
					if (mysqli_stmt_affected_rows($lstmt) == 1) {	//login data insert successful
						...
						$ds = "INSERT INTO daily_success_steps (rep_id) VALUES (?)";
						...
						$uss = "INSERT INTO ultimate_success_steps (repid) VALUES (?)";
						...
						/*if (mysqli_stmt_affected_rows($ustmt) != 1) {
							echo '<p>' . mysqli_stmt_error($ustmt) . '</p>';
							exit();
						}*/
	  ...
	  <div class="rep_signup_container roundcorners opacity95">
			<h2>VFGCONTACT.COM - Rep Signup</h2>
			<div class="webform">
				<form name="rep_signupform" id="rep_signupform" action="rep_signup.php" method="POST" >
					<span>NOTE: This contact system uses Google's Calendar and Email functions.  You must have a gmail account to use this system.
					<br /><br />Provide your name, phone, and the timezone you are located in.</span>
					<ul>
						<li>
					...
					<span>Enter your VFG ID and tell us if this is your temporary or permanent VFG ID.<br /><b>IT IS CRITICAL</b> to make the proper designation here!</span>
					<ul>
						<li>
						...
						<li>
							...
							<?php 
								$selected_vfgtype = "";
								if (isset($_POST['vfgtype'])){
									$selected_vfgtype = $_POST['vfgtype'];
								} else { 
									$selected_vfgtype = 'T';
								}
							?>
							<select id="vfgtype" name="vfgtype">
								<?php
									foreach($vfgid_type as $id=>$name){
										if($selected_vfgtype == $id){
											$sel = 'selected="selected"';
										}
										else{
											$sel = '';
					...
					<span>Create your password. You will use your VFG ID and password to log into vfgcontact.com.</span>
					<ul>
						<li>
		...
		</div> <!-- close rep_signup_container -->
		...
		<div id="rep_signup_messages" class="roundcorners opacity95">
			<?php
				...
				if (!empty($errors)) {
					...
					// DUPLICATE GMAIL FLAG
					if ($duplicate_gmail){
						echo '<p><b>You have entered a GMAIL account that already exists in the system.</b></p>';
						echo '<p>Have you previously signed up using a TEMPORARY VFG ID, and you are trying to create
								a new account with your PERMANENTLY ASSIGNED VFG ID?</p>';
						echo '<p align="center">
								<form name="dup_email" id="dup_email" action="duplicate_gmail.php" method="POST">
								<input type="submit" value="Yes, I signed up with a TEMP VFG ID." />
								</form>
							  </p>';
						echo '<p>If you <b>have NOT</b> previously signed up, then the Gmail address you are trying
								to use belongs to another rep. Check to make sure you entered your Gmail correctly.</p>';
						
						echo '<p><br /><br />Contact support if the scenario above doesn\'t apply, and you believe there is an issue with the Gmail Account you are
								trying to sign up with.</p>';
					}
				}
