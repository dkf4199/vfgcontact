...
$_SESSION['from_page'] = 'rep_referral_sources.php';
$_SESSION['back_btn_text'] = 'Back To Referral Sources';
...
<title>VFG Contact - Referral Sources</title>
<!-- CSS -->
			...
			<?php
				...
				*/
				...
				/*$q = "SELECT a.contact_id, a.firstname, a.lastname, a.email, 
					  ...
					  AND a.disposition = 'RS'
					  OR a.disposition = 'BR'
					  ORDER BY $order_by 
					  LIMIT $start, $display";	
				*/
				$q = "SELECT a.contact_id, a.firstname, a.lastname, a.email, 
							a.phone, a.disposition, b.disposition_desc
					  FROM contacts a
					  INNER JOIN ref_disposition b ON a.disposition = b.disposition_code
					  WHERE a.rep_id = '$repsid'
					  AND a.disposition IN ('RS','BR')
					  LIMIT $start, $display";
				...
				if ($r) { // If it ran OK, display the records.
					...
					echo '<table id="csstable">
						<tr>
							<th colspan=8>YOUR REFERRAL SOURCES ('.$_SESSION['total_rs_recs'].' total)</th>
						</tr>
						<tr>
					...
					while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
						...
						if ($row['phone'] != ''){
							...
							//12/29/2013 move event to tcs_modal_handlers in document.ready
							echo '<td>'.$row['phone'].'<a onclick="dialerCall( \''.$row['phone'].'\');" id="dialerlink" class="phonelist_modal" href="#">
										<img src="./images/smallicons/remotedial.png" class="phonelink"/></a></td>';
						} else {
							echo '<td>'.$row['phone'].'</td>';
						}
						
						echo '<td>'.$row['disposition_desc'].'</td>
							 </tr>';
					...
					if ($pages > 1) {
						...
						if ($current_page != 1) {
							echo '<a href="rep_referral_sources.php?s='.($start - $display).'&p='.$pages.'">Previous</a>&nbsp;';
						...
						for ($i = 1; $i <= $pages; $i++){
							if ($i != $current_page){
								echo '<a href="rep_referral_sources.php?s='.(($display * ($i - 1))).'&p='.$pages.'">'.$i.'</a>&nbsp;';
						...
						if ($current_page != $pages){
							echo '&nbsp;<a href="rep_referral_sources.php?s='.($start + $display).'&p='.$pages.'">Next</a>';
							//echo '<a href="rep_view_contact.php?&s='.($start + $display).'&p='.
							//	$pages.'">Next</a> ';
						}
					...
					//refresh button
					echo '<p align="center">
						<form action="rep_referral_sources.php" method="GET" >
					...
					// Public message:
					echo '<p>Your referral sources could not be retrieved. We will fix the problem shortly.</p>';
					//echo '</div>';
