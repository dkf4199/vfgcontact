...
<div id="wrapper">
	...
	<div class="maincontent">
		...
		<div style="text-align:center;">
			<form method="get" action="search_reps_last.php">
		...
		<div id="ajax_viewcontacts" class="opacity80">
			<?php
				...
				$_SESSION['from_page'] = 'search_reps_last.php';
				$_SESSION['back_btn_text'] = 'Back To Search';
				...
				if ($searchstr != ''){
					...
					if ($r) { // If it ran OK, display the records.
						...
						if ($pages > 1) {
							...
							if ($current_page != 1) {
								echo '<a href="search_reps_last.php?s='.($start - $display).'&p='.$pages.'">Previous</a>&nbsp;';
							...
							for ($i = 1; $i <= $pages; $i++){
								if ($i != $current_page){
									echo '<a href="search_reps_last.php?s='.(($display * ($i - 1))).'&p='.$pages.'">'.$i.'</a>&nbsp;';
							...
							if ($current_page != $pages){
								echo '&nbsp;<a href="search_reps_last.php?s='.($start + $display).'&p='.$pages.'">Next</a>';
								//echo '<a href="rep_view_contact.php?&s='.($start + $display).'&p='.
								//	$pages.'">Next</a> ';
							}
